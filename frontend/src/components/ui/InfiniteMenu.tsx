...
class IcosahedronGeometry extends Geometry {
  constructor() {
    ...
    this.addVertex(-1,t,0, 1,t,0, -1,-t,0, 1,-t,0, 0,-1,t, 0,1,t, 0,-1,-t, 0,1,-t, t,0,-1, t,0,1, -t,0,-1, -t,0,1)
        .addFace(0,11,5, 0,5,1, 0,1,7, 0,7,10, 0,10,11, 1,5,9, 5,11,4, 11,10,2, 10,7,6, 7,1,8,
...
function createProgram(gl, sources, tfVaryings, attribLocs) {
  ...
  [gl.VERTEX_SHADER, gl.FRAGMENT_SHADER].forEach((t, i) => {
    ...
    if (s) gl.attachShader(prog, s);
  });
  if (tfVaryings) gl.transformFeedbackVaryings(prog, tfVaryings, gl.SEPARATE_ATTRIBS);
  if (attribLocs) for (const a in attribLocs) gl.bindAttribLocation(prog, attribLocs[a], a);
  gl.linkProgram(prog);
...
function createTexture(gl, min, mag, wrapS, wrapT) {
...
class ArcballControl {
  ...
  constructor(canvas, cb) {
    ...
    this._rv = 0; this._cq = quat.create();
  ...
  update(dt, targetFPS = 16) {
    ...
    if (this.isPointerDown) {
      ...
      quat.slerp(this.pointerRotation, this.pointerRotation, this.IDENTITY_QUAT, .1 * ts);
    ...
    quat.slerp(this._cq, this._cq, cq, .8 * ts);
...
class InfiniteGridMenu {
  ...
  run(time = 0) {
    ...
    this.#dFrames = this.#dt / this.TARGET_FPS; this.#frames += this.#dFrames;
  ...
  #onControlUpdate(dt) {
    const ts = dt / this.TARGET_FPS + .0001;
    let damping = 5 / ts, targetZ = 3 * this.scaleFactor;
    ...
    if (!this.control.isPointerDown) {
      const ni = this.#findNearest();
      const itemIdx = ni % Math.max(1, this.items.length);
...
export default function InfiniteMenu({ items = [], scale = 1.0, onSelect }: Props) {
  ...
  useEffect(() => {
    ...
    const handleActive = (index: number) => {
      ...
      setActiveItem(arr[index % arr.length]);
    };

    const sketch = new InfiniteGridMenu(
      canvas, items.length ? items : defaultItems,
      handleActive, setIsMoving, (sk) => sk.run(), scale,
    );
    ...
    <div className="relative w-full h-full">
      ...
      {activeItem && (
        <>
          <h2
            style={{
              position: 'absolute', left: '1.6em', top: '50%',
              ...
              transition: isMoving ? 'opacity 100ms' : 'opacity 500ms',
              userSelect: 'none',
            }}
          >
            {activeItem.title}
          ...
          <p
            style={{
              position: 'absolute', maxWidth: '10ch', fontSize: '1.2rem',
              top: '50%', right: '1%', color: '#748D92',
              opacity: isMoving ? 0 : 1,
              transform: isMoving ? 'translate(-60%, -50%)' : 'translate(-90%, -50%)',
              transition: isMoving ? 'opacity 100ms, transform 100ms' : 'opacity 500ms, transform 500ms',
              userSelect: 'none',
            }}
          >
          ...
          <div
            ...
            style={{
              ...
              background: '#124E66',
              border: '3px solid #748D92',
              borderRadius: '50%',
              cursor: 'pointer',
              opacity: isMoving ? 0 : 1,
              transform: isMoving ? 'translate(-50%, 0) scale(0)' : 'translate(-50%, 0) scale(1)',
              bottom: isMoving ? -80 : '3.8em',
              transition: isMoving ? 'opacity 100ms, transform 100ms, bottom 100ms' : 'opacity 500ms, transform 500ms, bottom 500ms',
              userSelect: 'none',
            }}
          >
            <span style={{ color: '#D3D9D4', fontSize: 26, position: 'relative', top: 2 }}>&#x2197;</span>
          </div>
        </>
      )}
    </div>
